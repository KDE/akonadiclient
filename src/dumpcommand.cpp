...
void DumpCommand::setupCommandOptions(QCommandLineParser *parser)
{
    ...
    parser->addOption(QCommandLineOption((QStringList() << "m"
                                                        << "maildir"),
                                         i18n("Dump email messages in maildir directory structure")));
    parser->addOption(QCommandLineOption((QStringList() << "a"
                                                        << "akonadi-categories"),
                                         i18n("Dump items with Akonadi category URLs, otherwise text names")));
    parser->addOption(QCommandLineOption((QStringList() << "f"
                                                        << "force"),
                                         i18n("Operate even if destination directory is not empty")));
...
int DumpCommand::initCommand(QCommandLineParser *parser)
{
    ...
    if (!checkArgCount(args, 1, i18nc("@info:shell", "No collection specified")))
        return InvalidUsage;
    if (!checkArgCount(args, 2, i18nc("@info:shell", "No destination directory specified")))
        return InvalidUsage;
...
void DumpCommand::onCollectionFetched(KJob *job)
{
    ...
    if (contentMimeTypes.isEmpty()) {
        ErrorReporter::fatal(i18nc("@info:shell", "Collection %1 cannot contain items", res->formattedCollectionName()));
        return;
    }
...
void DumpCommand::onItemsFetched(KJob *job)
{
    ...
    if (items.isEmpty()) {
        ErrorReporter::fatal(i18nc("@info:shell", "Collection %1 contains no items", resolveJob()->formattedCollectionName()));
...
void DumpCommand::writeItem(const Akonadi::Item &item, const QString &parent)
{
    ...
    if (!dir.exists()) // ensure that it exists
    {
        ...
        if (isDryRun()) // not actually doing anything
        {
            ...
            {
                ErrorReporter::fatal(i18nc("@info:shell", "Cannot create directory '%1/%2'", parentDir.canonicalPath(), dirName));
                Q_EMIT finished(RuntimeError);
                return;
            }
        }
    ...
    if (!isDryRun()) {
        ...
        if (mimeType == "text/directory" || mimeType == "text/vcard") { // need to fix up tags?
