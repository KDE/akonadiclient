...
void FoldersCommand::start()
{
    if (mOperationMode == ModeRestore) {
        ...
        // this step and go straight away to use the saved lists.
        processChanges();
        return;
    }
...
bool FoldersCommand::readOrSaveLists()
{
    ...
    if (mOperationMode == ModeCheck) // check mode, create current file
    {
        // Check mode, so save the current list of folders that was obtained
        // from the Akonadi server to a "current" data file.  This will be later
        // used by a restore operation because by then the list will not be
        // available from the server.
        const QString currFileName = findSaveFile("currentfolders.dat", true);
        qDebug() << "current ->" << currFileName;
    ...
    } else // restore mode, read current file
    {
        // TODO: not needed if 'restore' using results of 'check'

        // Restore mode, so read the "current" list of folders from the saved
        // data file.
        const QString currFileName = findSaveFile("currentfolders.dat", false);
        qDebug() << "restore <-" << currFileName;
        QFile currFile(readFileName, this);
        if (!currFile.open(QIODevice::ReadOnly)) {
            Q_EMIT error(i18nc("@info:shell", "Cannot read current list from '%1'", currFile.fileName()));
            Q_EMIT finished(RuntimeError);
            return (false);
        }

        readSavedPaths(&currFile, &mCurPathMap); // populates mCurPathMap
    }
...
void FoldersCommand::processChanges()
{
    ...
    // Process each application configuration file in turn, as defined by the
    // change data.
    ...
    for (const QString &configName : std::as_const(configNames)) {
        ...
        const QStringList dirFiles = configDir.entryList();
        for (const QString &configFile : std::as_const(dirFiles)) {
            std::cerr << std::endl;
            ...
            KConfig newConfig(configFile + "_new", KConfig::SimpleConfig);
    ...
    if (mOperationMode == ModeCheck) // check mode, further instructions
    {
        std::cerr << std::endl
...
void FoldersCommand::populateChangeData()
{
    ...
    ErrorReporter::progress(i18nc("@info:shell", "Defined %1 change data patterns for %2 config files", mChangeData.count(), mChangeData.uniqueKeys().count()));
}
